...
function App() {
  const [currentList,setCurrentList]=useState(MoviesList)
  
  function handleGenre(type){
    
   setCurrentList(MoviesList.filter((element)=>{
    return element.genre===type;
   }))
   console.log("currentlist is",currentList,type);
  }

  return (
    <>
    <h1 style={{textAlign:"center"}}>Top 15 movie of all Time</h1>
     <GenreFilter handleGenre={handleGenre} />
      <MovieList currentList={currentList} />
    </>
  )
}
